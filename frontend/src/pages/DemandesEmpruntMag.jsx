...
import { getDemandesEmprunt, traiterDemandeEmprunt } from '../services/magasinierService'
...
export default function DemandesEmpruntMag() {
  ...
  const fetchDemandes = async () => {
    try {
      const data = await getDemandesEmprunt()
      setDemandes(data)
    } catch (err) {
  ...
  const handleAction = async (id, statut) => {
    try {
      setActionLoading(id)
      await traiterDemandeEmprunt(id, { statut, valideur_id: userId })
    ...
    <div className={`p-6 ${darkMode ? 'text-white' : 'text-slate-800'}`}>
      ...
      {countEnAttente > 0 && (
        <div className="mb-4 px-4 py-3 rounded-lg border text-sm flex items-center gap-2 bg-yellow-50 border-yellow-200 text-yellow-800">
          ...
          <span><strong>{countEnAttente}</strong> demande(s) d'emprunt en attente</span>
        </div>
      )}
      ...
      <div className="rounded-xl overflow-hidden border border-gray-200">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-slate-400">Aucune demande</td></tr>
            ) : filtered.map((d, i) => (
              <tr key={d.id} className={`border-t border-gray-100 ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-slate-50`}>
                ...
                <td className="px-4 py-3 text-slate-500">{d.quantite || 1}</td>

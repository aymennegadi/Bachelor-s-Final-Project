    ...
    <div className={`p-6 ${darkMode ? 'text-white' : 'text-slate-800'}`}>
      ...
      {countEnAttente > 0 && (
        <div className={`mb-4 px-4 py-3 rounded-lg border text-sm flex items-center gap-2
          ${darkMode ? 'bg-yellow-900/20 border-yellow-700 text-yellow-400' : 'bg-yellow-50 border-yellow-200 text-yellow-800'}`}>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 ? (
              ...
              <tr key={d.id} className={`border-t transition-colors
                ${darkMode
